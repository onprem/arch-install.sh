...
function browser {
	...
	read -r -p "Instsll chromium? " chrom
	case "$chrom" in
	    [yY][eE][sS]|[yY]) 
...
function installsteam {
	...
	case "$isteam" in
	    [yY][eE][sS]|[yY])
	        arch-chroot /mnt bash -c "pacman -S steam lib32-gtk2 lib32-gtk3 lib32-libpulse lib32-libvdpau lib32-libva lib32-libva-vdpau-driver lib32-openal && exit"
	        ;;
	    *)
	        ;;
	esac
